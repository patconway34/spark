...
**Why:** the terminals are same-origin iframes (`<spark-host>/term/*`).
...
1. Cloudflare Access on the Spark hostname (covers `/term/*` — same host),
...
(`terminal2.<domain>` etc.), the basic-auth prompt DID show on mobile
